...
export class CardsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'position', 'name', 'address', 'email', 'department', 'phone', 'issuedOn'];
  dataSource = new MatTableDataSource<any>(this.userLists);
  ...
  isAllSelected() {
    ...
    // // console.log(this.selection.selected, 'asdjhashd');
    // return numSelected === numRows;
    const numSelected = this.selection.selected.length;
    const page = this.dataSource.paginator?.pageSize;
    this.tablePageSize = page;
  ...
  selectRows() {
    for (let index = 0; index < this.tablePageSize; index++) {
  ...
  uploadFiles() {
    ...
    if(this.selectedStamp.length > 0) {
  ...
  createZip() {
    console.log(this.generatedImage, 'KATI OTA AAYO')
    var zip = new JSZip();
    // zip.folder("cards");
    const chunkSize = 1;
    for (let i = 0; i < this.generatedImage.length; i += chunkSize) {
  ...
  individualDownloadP() {
    ...
    console.log(this.selection.selected, 'INDIVIDUAL PORTRAIT')

    const chunkSize = 1;
    for (let i = 0; i < this.selection.selected.length; i += chunkSize) {
      const chunk = this.selection.selected.slice(i, i + chunkSize);
      console.log(chunk, 'Chunk')

      chunk.forEach(async item => {
        const canvas = await html2canvas(document.getElementById('card'+item.cardnumber) as HTMLImageElement);
        ...
        console.log('card'+item.cardnumber,img, this.selection.selected.length,this.generatedImage.length, 'n');

        if (this.selection.selected.length === this.generatedImage.length) {
          this.createZip();
        }
  ...
  async individualDownload() {
    this.downloadLoading = true;
    console.log(this.selection.selected, 'asjbaskjdbaksjd')
    ...
    for (let i = 0; i < this.selection.selected.length; i += chunkSize) {
      ...
      chunk.forEach(async item => {
        const canvas = await html2canvas(document.getElementById(item.userid) as HTMLImageElement);
        // canvas.style.display = "block";
        // document.body.appendChild(canvas);
        const image = canvas
          .toDataURL("image/png")
